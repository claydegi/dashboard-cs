...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('stat-card-score').addEventListener('click', () => {
        window.location.href = `/crm-score?regione=PIEMONTE&key=${ADMIN_KEY}`;
    });
...
function renderTableBody() {
    ...
    filtered.forEach((c, idx) => {
        ...
        const waIcon = c.gruppo_whatsapp
            ? `<span class="crm-wa-icon crm-wa-active" title="Gruppo WhatsApp">${waSvg}</span>`
            : `<span class="crm-wa-icon crm-wa-toggle" title="Clicca per aggiungere a WhatsApp" onclick="toggleWhatsApp(${c.id}, this)">${waSvg}</span>`;
...
function renderLeadTable() {
    ...
    filtered.forEach((c, idx) => {
        ...
        const waIconLead = c.gruppo_whatsapp
            ? `<span class="crm-wa-icon crm-wa-active" title="Gruppo WhatsApp">${waSvgLead}</span>`
            : `<span class="crm-wa-icon crm-wa-toggle" title="Clicca per aggiungere a WhatsApp" onclick="toggleWhatsApp(${c.id}, this)">${waSvgLead}</span>`;
...
async function toggleWhatsApp(contattoId, spanEl) {
    if (!confirm('Segnare questo contatto come membro del gruppo WhatsApp?')) return;
    ...
    try {
        ...
        const contatto = allContatti.find(c => c.id === contattoId);
        if (contatto) contatto.gruppo_whatsapp = true;

        // Aggiorna icona: da grigia cliccabile a verde fissa
        spanEl.classList.remove('crm-wa-toggle');
        spanEl.classList.add('crm-wa-active');
        spanEl.removeAttribute('onclick');
        spanEl.title = 'Gruppo WhatsApp';
