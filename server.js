...
app.get('/api/freelancer/mock-status', (req, res) => {
    res.json({ mock_mode: FREELANCER_MOCK_MODE });
});

// Mock data store (simula stato API)
const mockDataStore = {
    projects: {},
    bids: {},
    threads: {},
    nextProjectId: 100000,
    nextBidId: 500000,
    nextThreadId: 700000
};

/**
 * Simula chiamate API Freelancer.com (per testing senza API reale)
 */
async function freelancerApiCallMock(method, endpoint, body = null) {
    console.log(`[MOCK API] ${method} ${endpoint}`, body ? JSON.stringify(body).substring(0, 100) : '');

    // Simula ritardo rete (100-300ms)
    await new Promise(resolve => setTimeout(resolve, 100 + Math.random() * 200));

    // POST /projects/0.1/projects/ - Pubblica progetto
    if (method === 'POST' && endpoint.includes('/projects/0.1/projects/')) {
        const projectId = mockDataStore.nextProjectId++;
        const seoUrl = body.title.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + projectId;

        mockDataStore.projects[projectId] = {
            id: projectId,
            title: body.title,
        ...
        // Genera 5-8 bid fake realistici
        const numBids = 5 + Math.floor(Math.random() * 4);
        mockDataStore.bids[projectId] = [];

        const freelancers = [
            { username: 'webdev_pro', rating: 9.2, reviews: 150, complete: 142, on_budget: 95, on_time: 92, country: 'India' },
            { username: 'designmaster', rating: 8.8, reviews: 89, complete: 85, on_budget: 88, on_time: 85, country: 'Ukraine' },
            { username: 'codewizard99', rating: 9.5, reviews: 220, complete: 210, on_budget: 98, on_time: 96, country: 'Pakistan' },
            { username: 'creative_ninja', rating: 7.9, reviews: 45, complete: 40, on_budget: 82, on_time: 78, country: 'Philippines' },
            { username: 'techguru_uk', rating: 9.0, reviews: 180, complete: 175, on_budget: 92, on_time: 90, country: 'United Kingdom' },
            { username: 'fastcoder', rating: 8.5, reviews: 120, complete: 110, on_budget: 85, on_time: 88, country: 'Bangladesh' },
            { username: 'qualitywork', rating: 9.3, reviews: 95, complete: 92, on_budget: 96, on_time: 94, country: 'Romania' },
            { username: 'budget_dev', rating: 7.2, reviews: 30, complete: 25, on_budget: 70, on_time: 65, country: 'India' }
        ];

        for (let i = 0; i < numBids; i++) {
            const freelancer = freelancers[i];
            const bidId = mockDataStore.nextBidId++;
            const bidderId = 1000 + i;

            // Budget realistico: 70-130% del budget massimo
            const budgetMax = body.budget.maximum;
            const amount = Math.round(budgetMax * (0.7 + Math.random() * 0.6));
            const period = 3 + Math.floor(Math.random() * 12); // 3-14 giorni

            mockDataStore.bids[projectId].push({
                id: bidId,
                bidder_id: bidderId,
                amount: amount,
                period: period,
                description: `I can complete this project with high quality. I have ${freelancer.reviews} reviews and ${freelancer.rating}/10 rating. Let's discuss the requirements.`,
                milestone_percentage: 50,
                submitdate: Math.floor(Date.now() / 1000) - Math.floor(Math.random() * 86400),
                user: {
                    id: bidderId,
                    username: freelancer.username,
                    display_name: freelancer.username.replace('_', ' ').toUpperCase(),
                    location: { country: { name: freelancer.country } },
                    reputation: {
                        entire_history: {
                            overall: freelancer.rating,
                            reviews: freelancer.reviews,
                            complete: freelancer.complete,
                            on_budget: freelancer.on_budget,
                            on_time: freelancer.on_time
                        }
                    }
                }
            });
        ...
        return { id: projectId, seo_url: seoUrl };
    }

    // GET /projects/0.1/bids/ - Lista bid
    if (method === 'GET' && endpoint.includes('/projects/0.1/bids/')) {
        const match = endpoint.match(/projects\[\]=(\d+)/);
        if (!match) return { bids: [], users: {} };

        const projectId = parseInt(match[1]);
        const bids = mockDataStore.bids[projectId] || [];

        const users = {};
        bids.forEach(bid => {
            users[bid.bidder_id] = bid.user;
        });

        return { bids, users };
    }

    // POST /projects/0.1/projects/{id}/ con action=award - Assegna progetto
    if (method === 'POST' && endpoint.includes('/projects/0.1/projects/') && body?.action === 'award') {
        const match = endpoint.match(/projects\/(\d+)/);
        const projectId = match ? parseInt(match[1]) : null;

        if (projectId && mockDataStore.projects[projectId]) {
            mockDataStore.projects[projectId].status = 'awarded';
            mockDataStore.projects[projectId].awarded_bid_id = body.bid_id;
        }

        return { id: body.bid_id, status: 'awarded' };
    }

    // POST /messages/0.1/threads/ - Crea thread messaggio
    if (method === 'POST' && endpoint.includes('/messages/0.1/threads/')) {
        const threadId = mockDataStore.nextThreadId++;
        const contextId = body.context;

        mockDataStore.threads[threadId] = {
            id: threadId,
    ...
    // GET /messages/0.1/threads/ - Lista thread
    if (method === 'GET' && endpoint.includes('/messages/0.1/threads/')) {
        const match = endpoint.match(/projects\[\]=(\d+)/);
        if (!match) return { threads: [] };

        const projectId = parseInt(match[1]);
        const threads = Object.values(mockDataStore.threads).filter(t => t.context === projectId);

        return { threads };
    }

    // GET /messages/0.1/threads/{id}/messages/ - Messaggi thread
    if (method === 'GET' && endpoint.includes('/messages/0.1/threads/') && endpoint.includes('/messages/')) {
        const match = endpoint.match(/threads\/(\d+)/);
        if (!match) return { messages: [] };

        const threadId = parseInt(match[1]);
        const thread = mockDataStore.threads[threadId];

        if (!thread) return { messages: [] };

        // Simula messaggi freelancer (50% chance ha risposto)
        if (Math.random() > 0.5 && thread.messages.length === 1) {
            thread.messages.push({
                id: 2,
                from_user: thread.members[0], // Freelancer
                message: "Thank you for choosing me! I'll start working on this right away. I'll keep you updated on the progress.",
                time_created: thread.messages[0].time_created + 3600
            });
        }
    ...
    throw new Error(`Mock non implementato per: ${method} ${endpoint}`);
}

async function freelancerApiCall(method, endpoint, body = null) {
    // Se mock mode attivo, usa mock
    if (FREELANCER_MOCK_MODE) {
        return freelancerApiCallMock(method, endpoint, body);
    }

    // Altrimenti chiamata API reale
